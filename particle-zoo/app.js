...
// Declared before the observer that marks it so the callback can never run in
// its temporal dead zone; particle-zoo/validate.js pins that order.
let labStaticDirty=true;
...
// Panels are revealed one semantic chunk per timer task. Every listed panel is
// asserted to own at least two chunks by the feature suite.
const PANEL_CHUNK_SELECTORS = {
...
// Single-letter tile symbols for the standard-model rows, and the labels for
// the self-conjugate row (both pinned against TILE_GROUPS in validate.js).
const TILE_SYMBOLS = {up:'u',down:'d',charm:'c',strange:'s',top:'t',bottom:'b',
...
const SELF_CONJUGATE_META = { photon:'γ = γ̄', zboson:'Z = Z̄', higgs:'H = H̄' };
function symbolFor(id){
  return TILE_SYMBOLS[id];
}
...
function rebuildStandardModelTiles(){
  ...
  Object.entries(TILE_GROUPS).forEach(([group, ids])=>{
    ...
    ids.forEach(id=>{
      ...
      yb.className='p-year';
      yb.textContent=shortYear(p.discovered);
      tile.appendChild(yb);
...
function renderContentReferences(tab){
  ...
  if(tab==='forces'){
    ...
    document.querySelectorAll('.bsm-card').forEach(card=>{
      ...
      replaceReferences(card, refs.bsm[key]);
    });
    ...
    document.querySelectorAll('.phen-card').forEach(card=>{
      ...
      replaceReferences(card, refs.phenomenon[key]);
    });
  } else if(tab==='lab'){
...
buildCanvas.addEventListener('click',e=>{
  ...
  if(best>=0){
    parts.splice(buildViz.particles[best].partIdx,1);
    buildComposites();
  }
});
...
function buildComposites(){
  ...
  mesons.forEach(m=>{
    ...
    m.info = MESON_TABLE[key];
    m.charge = QUARK_CHARGE[m.q] + QUARK_CHARGE[m.aq];
  });
  ...
  // 7) Flat hit-test list: every drawn constituent points back at the tray part
  //    it came from, so clicking the canvas removes exactly one piece.
  const partIndexPool = {};
  parts.forEach((flavour, index)=>{
    partIndexPool[flavour] = partIndexPool[flavour] || [];
    partIndexPool[flavour].push(index);
  });
  const takePartIndex = flavour => partIndexPool[flavour].shift();
  const particles = [];
  nucleons.forEach(n=>n.quarkPos.forEach(q=>particles.push({ x:q.x, y:q.y, partIdx: takePartIndex(q.type) })));
  mesons.forEach(m=>{
    particles.push({ x:m.qPos.x,  y:m.qPos.y,  partIdx: takePartIndex(m.q) });
    particles.push({ x:m.aqPos.x, y:m.aqPos.y, partIdx: takePartIndex(m.aq) });
  });
...
function drawBuild(){
  ...
  if(hasAtom){
    ...
    bctx.textAlign='left';
    bctx.fillText((LOCALES[window.CURRENT_LANG])['builder.label.shell'], cx-atomR+10, cy-atomR-8);
  }
  ...
  if(N>=2){
    ...
    bctx.fillText((LOCALES[window.CURRENT_LANG])['builder.label.nucleus'], cx, cy-R-8);
...
var IX_DEFS = [
  ...
  { id:'beta', group:'common', tag:'weak', tagKey:'ix.tag.weak',
    ...
    build(){
      ...
      return { svg:s, anim(el,t){ const w=el.querySelector('.ix-w'); ixDashPhase(w,t*2); } };
    }
  },
  ...
  { id:'annih', group:'common', tag:'em', tagKey:'ix.tag.em',
    ...
    build(){
      ...
      return { svg:s, anim(el,t){
        ...
        p1.setAttribute('d', ixWavy(xV,yV,IX_W-IX_PAD_X,25,-t*4));
        p2.setAttribute('d', ixWavy(xV,yV,IX_W-IX_PAD_X,85,-t*4));
      }};
    }
  },
  ...
  { id:'pair', group:'common', tag:'em', tagKey:'ix.tag.em',
    ...
    build(){
      ...
      return { svg:s, anim(el,t){
        ...
        p.setAttribute('d', ixWavy(IX_PAD_X,yV,xV,yV,-t*4));
      }};
    }
  },
  ...
  { id:'higgs', group:'common', tag:'higgs', tagKey:'ix.tag.higgs',
    ...
    build(){
      ...
      const vx=xLoop+28;
      return { svg:s, anim(el,t){
        const h=el.querySelector('.ix-h'); ixDashPhase(h,t*2);
        const p1=el.querySelector('.ix-ph1'), p2=el.querySelector('.ix-ph2');
        p1.setAttribute('d', ixWavy(vx,yC-8,xR,25,-t*4));
        p2.setAttribute('d', ixWavy(vx,yC+8,xR,85,-t*4));
      }};
    }
  },
  ...
  { id:'gluon', group:'common', tag:'strong', tagKey:'ix.tag.strong',
    ...
    build(){
      ...
      return { svg:s, anim(el,t){
        ...
        g.setAttribute('d', ixCurly(vT_x,vT_y,vB_x,vB_y,t*4));
      }};
    }
  },
  ...
  { id:'ex-strong', group:'exchange', tag:'strong', tagKey:'ix.tag.strong',
    ...
    build(){
      ...
      return { svg:s, anim(el,t){
        const g=el.querySelector('.ix-g');
        g.setAttribute('d', ixCurly(vT[0],vT[1],vB[0],vB[1],t*4));
      }};
    }
  },
  ...
  { id:'ex-em', group:'exchange', tag:'em', tagKey:'ix.tag.em',
    ...
    build(){
      ...
      return { svg:s, anim(el,t){
        const p=el.querySelector('.ix-ph');
        p.setAttribute('d', ixWavy(vT[0],vT[1],vB[0],vB[1],t*4));
      }};
    }
  },
  ...
  { id:'ex-grav', group:'exchange', tag:'grav', tagKey:'ix.tag.grav',
    ...
    build(){
      ...
      return { svg:s, anim(el,t){
        ...
        g1.setAttribute('d', ixWavy(vT[0]-3,vT[1],vB[0]-3,vB[1],t*3,4,4));
        g2.setAttribute('d', ixWavy(vT[0]+3,vT[1],vB[0]+3,vB[1],t*3+Math.PI,4,4));
      }};
    }
  },
  ...
  { id:'ex-higgs', group:'exchange', tag:'higgs', tagKey:'ix.tag.higgs',
    ...
    build(){
      ...
      return { svg:s, anim(el,t){ const h=el.querySelector('.ix-h'); ixDashPhase(h,t*2); } };
    }
  },
  ...
  { id:'compton', group:'rare', tag:'em', tagKey:'ix.tag.em',
    ...
    build(){
      ...
      return { svg:s, anim(el,t){
        const p1=el.querySelector('.ix-ph1'), p2=el.querySelector('.ix-ph2');
        p1.setAttribute('d', ixWavy(xL,30,v1[0],v1[1],-t*4));
        p2.setAttribute('d', ixWavy(v2[0],v2[1],xR,30,-t*4));
      }};
    }
  },
  ...
  { id:'bremss', group:'rare', tag:'em', tagKey:'ix.tag.em',
    ...
    build(){
      ...
      return { svg:s, anim(el,t){
        const p=el.querySelector('.ix-ph');
        p.setAttribute('d', ixWavy(xV,yC,xR,25,-t*4));
      }};
    }
  },
  ...
  { id:'gravwave', group:'rare', tag:'grav', tagKey:'ix.tag.grav',
    ...
    build(){
      ...
      return { svg:s, anim(el,t){
        const g1=el.querySelector('.ix-gr1'), g2=el.querySelector('.ix-gr2');
        g1.setAttribute('d', ixWavy(46,yC-2,IX_W-46,yC-2,-t*3,4,5));
        g2.setAttribute('d', ixWavy(46,yC+2,IX_W-46,yC+2,-t*3+Math.PI,4,5));
      }};
    }
  },
];
...
function labInitConfinement(){
  const c = requireElement('confCanvas');
...
function labInitDetector(){
  const c = requireElement('detCanvas');
  const { ctx, w, h } = labSizeCanvas(c);
...
function labRebuildDetectorPicker(){
  ...
  const picker = requireElement('detPicker');
  picker.innerHTML = '';
  ...
  const leg = requireElement('detLegend');
...
function drawCalCells(ctx, cx, cy, r0, r1, ang, halfWidth, color, kind, colsOverride){
  ...
  for(let iRow=0; iRow<rows; iRow++){
    ...
    for(let iCol=0; iCol<cols; iCol++){
      ...
      const centrality = 1 - Math.abs((iCol/(cols-1))-0.5)*2;
      const rowFactor = 1 - iRow/rows*0.4;
      const alpha = (kind==='mip' ? 0.4 : 0.35 + centrality*0.55) * rowFactor;
      const angC = ang - halfWidth + (iCol/(cols-1))*angSpread;
...
function labInitHiggs(){
  const c = requireElement('higgsCanvas');
  const { ctx, w, h } = labSizeCanvas(c);
...
function labRebuildHiggsPicker(){
  ...
  const picker = requireElement('higgsPicker');
  picker.innerHTML = '';
...
function labInitFeyn(){
  const c = requireElement('feynCanvas');
  const { ctx, w, h } = labSizeCanvas(c);
  const prev = LAB.feyn || {};
  ...
  const clr = requireElement('feynClear');
  clr.onclick = ()=>{ LAB.feyn.vertices = []; LAB.feyn.edges = []; };
  const ex = requireElement('feynExample');
  ex.onclick = ()=>{
...
function labRebuildFeynPicker(){
  ...
  const picker = requireElement('feynPicker');
  picker.innerHTML = '';
  ...
  const leg = requireElement('feynLegend');
  leg.innerHTML = `
...
function labInitDecay(){
  const c = requireElement('decayCanvas');
  const { ctx, w, h } = labSizeCanvas(c);
  const prev = LAB.decay || {};
  LAB.decay = { ctx, w, h, canvas:c, current: prev.current || 'τ⁻', tree:null, speed:1 };
  labRebuildDecayPicker();
  const btn = requireElement('decayRestart');
  btn.onclick = ()=> labDecayStart();
  const spd = requireElement('decaySpeed');
  spd.oninput = ()=>{ LAB.decay.speed = parseFloat(spd.value); };
  LAB.decay.speed = parseFloat(spd.value);
  const leg = requireElement('decayLegend');
  const dict = LOCALES[window.CURRENT_LANG];
...
function labRebuildDecayPicker(){
  ...
  const picker = requireElement('decayPicker');
  picker.innerHTML='';
  DECAY_STARTERS.forEach(k=>{
    const b = document.createElement('button');
    b.innerHTML = `<b style="color:${DECAY_COLORS[k]}">${k}</b>`;
...
function labDecayStart(){
  ...
  const buildChildren = (node)=>{
    ...
    const ch = pickChannel(node.name);
...
function labDrawDecay(){
  ...
  const walkNodes = (n)=>{
    ...
    if(stable){
      ...
      ctx.fillText(dict['lab.decay.stable'], n._x, n._y+18);
    }
  ...
  const tau = `τ ≈ ${T0.tau.toExponential(1)} s`;
...
function labInitOsc(){
  const c = requireElement('oscCanvas');
  const { ctx, w, h } = labSizeCanvas(c);
  ...
  if(ctrls && !ctrls.dataset.built){
    ...
    const bind = (id, fn)=>{ const el=requireElement(id); el.oninput=fn; };
...
function labRebuildOscLegend(){
  const leg = requireElement('oscLegend');
  ...
  leg.innerHTML = `
    ...
    <span class="ll-item" style="cursor:pointer" id="oscSrcToggle">🔄 ${dict['lab.osc.source']}: ${src}</span>
    <div style="flex-basis:100%;color:#8b93b3;font-size:11px;margin-top:6px">${dict['lab.osc.legend']}</div>`;
  const tog = requireElement('oscSrcToggle');
  tog.onclick = ()=>{ OSC.source = (OSC.source+1)%3; labRebuildOscLegend(); };
}
...
function labInitPDF(){
  const c = requireElement('pdfCanvas');
  const { ctx, w, h } = labSizeCanvas(c);
  ...
  if(ctrls && !ctrls.dataset.built){
    ...
    ctrls.innerHTML = `
      ...
      <label class="switch"><input type="checkbox" id="pdfShowG" checked><span data-i18n="lab.pdf.showg">show gluon (÷10)</span></label>`;
...
function labRebuildPDFLegend(){
  const leg = requireElement('pdfLegend');
  const dict = LOCALES[window.CURRENT_LANG];
  leg.innerHTML = `
...
function labDrawPDF(){
  ...
  const drawCurve = (name, color, div)=>{
    ...
    for(let px=0; px<=plotW; px++){
      ...
      const v = pdfXf(name, x, PDF.logQ2) / div;
      const py = yToPx(v);
      if(px===0) ctx.moveTo(padL+px, py); else ctx.lineTo(padL+px, py);
    }
    ctx.stroke();
...
function labInitEVD(){
  const c = requireElement('evdCanvas');
  const { ctx, w, h } = labSizeCanvas(c);
  const prev = LAB.evd || {};
  LAB.evd = {
    ctx, w, h, canvas:c,
  ...
  const leg = requireElement('evdLegend');
  leg.innerHTML = `
...
function labRebuildEVDPicker(){
  ...
  const picker = requireElement('evdPicker');
  picker.innerHTML = '';
...
function labInitCons(){
  const c = requireElement('consCanvas');
  const { ctx, w, h } = labSizeCanvas(c);
  const prev = LAB.cons || {};
  LAB.cons = { ctx, w, h, canvas:c, R:prev.R||[], P:prev.P||[], example:prev.example||0, _rects:[] };
  ...
  if(ctrls && !ctrls.dataset.built){
    ...
    sel.onchange = ()=>{ LAB.cons.example = parseInt(sel.value); };
  ...
  const leg = requireElement('consLegend');
  leg.innerHTML = `
...
function labInitRun(){
  const c = requireElement('runCanvas');
  const { ctx, w, h } = labSizeCanvas(c);
...
function labRebuildRunLegend(){
  const leg = requireElement('runLegend');
  leg.innerHTML = `
...
function fixBigBangLink(){
  const a = requireElement('bigBangLink');
