...
const LOCALES = {
  'en': {
    ...
    'lab.evd.p': 'A real collider event, projected into the transverse plane of an ATLAS/CMS-style detector. Each track carries measured (pT, η, φ, ID). Pick an event, hover a track for details, toggle detector layers, or rotate the view.',
    ...
    'lab.cons.p': 'Every particle process must conserve certain quantities. Drag reactants and products into the two panels; the checker verifies electric charge, baryon number, lepton flavour numbers (Lₑ/L_μ/L_τ), strangeness, colour, and 4-momentum, then reports which force (if any) can mediate the process — or which law it violates.',
    ...
    'lab.run.p': 'In quantum field theory, coupling constants aren\'t constant — they run with the energy scale μ. Plotted here: α_em, α_s (strong), and sin²θ_W from 1 GeV to 10¹⁹ GeV, evolved with 1-loop β-functions. Toggle "MSSM particle content" to see the couplings nearly unify around 10¹⁶ GeV — the classic Grand Unification hint.',
    ...
    'lab.run.legend': 'Plotted as 1/α_i vs log₁₀(μ/GeV). SM (dashed) — the three lines miss each other by ~3%. MSSM (solid) — with SUSY partners above M_SUSY ≈ 1 TeV, the lines cross within ~1% near 2×10¹⁶ GeV.',
  ...
  'zh-CN': {
    ...
    'lab.evd.h': '8 · 交互式对撞事件',
    'lab.evd.tag': '事件展示',
    'lab.evd.p': '一个真实的对撞事件,投影到 ATLAS/CMS 风格探测器的横截面。每条径迹都带有测量到的 (pT, η, φ, 粒子 ID)。选一个事件、悬停某条径迹查看细节、切换探测层或旋转视角。',
    'lab.evd.rotate': '旋转',
    'lab.evd.tracker': '径迹室',
    'lab.evd.ecal': 'ECAL',
    'lab.evd.hcal': 'HCAL',
    ...
    'lab.cons.h': '9 · 守恒律检查器',
    'lab.cons.tag': '选择定则',
    'lab.cons.p': '每一次粒子反应都必须守恒若干量。把反应物拖到左边,产物拖到右边;检查器会验证电荷、重子数、三个轻子味数 (Lₑ/L_μ/L_τ)、奇异数、色荷和四动量,然后告诉你能通过哪种相互作用发生——或者违反了哪一条守恒律。',
    ...
    'lab.run.h': '10 · 跑动耦合常数',
    'lab.run.tag': '重整化群方程',
    'lab.run.p': '量子场论中的"耦合常数"并非常数——它们随能标 μ 而变化。这里画出 α_em、α_s(强)、sin²θ_W 从 1 GeV 到 10¹⁹ GeV 的演化,使用 1 圈图 β 函数。打开"MSSM 粒子谱"开关,你会看到三条线在约 10¹⁶ GeV 处几乎相交——这就是大统一理论 (GUT) 的经典暗示。',
    'lab.run.mssm': 'MSSM 粒子谱',
    'lab.run.show1': 'α₁(超荷,GUT 归一化)',
    'lab.run.show2': 'α₂(SU(2)_L)',
    'lab.run.show3': 'α₃(SU(3),强)',
    'lab.run.legend': '纵轴为 1/α_i,横轴为 log₁₀(μ/GeV)。标准模型(虚线)三条线彼此相差约 3%。MSSM(实线):在 M_SUSY ≈ 1 TeV 以上加入超对称伙伴,三条线在约 2×10¹⁶ GeV 处以 ~1% 精度会合。',
    'lab.run.axisx': 'log₁₀(μ / GeV)',
    'lab.run.axisy': '1 / α_i',
    // 画布覆盖文字
