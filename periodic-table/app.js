...
function rerenderGrid(){
  ...
  LAYOUT.forEach((row, rowIdx)=>{
    ...
    // Expand La-Lu / Ac-Lr placeholders into their actual Z ranges so
    // period 6 reads all 32 elements (Cs..Ba, La..Lu, Hf..Rn) and
    // period 7 reads Fr..Ra, Ac..Lr, Rf..Og.
    const rowZs = [];
    row.forEach(cell=>{
    ...
    speak.addEventListener('click', (e)=>{
      ...
      const est = rowZs.length * (lang==='zh-CN' ? 700 : 900) + 500;
      setTimeout(()=>speak.classList.remove('playing'), est);
    });
  ...
  F_BLOCK.forEach((row, fRowIdx)=>{
    ...
    speak.addEventListener('click', (e)=>{
      ...
      speak.classList.add('playing');
      const est = row.length * (lang==='zh-CN' ? 700 : 900) + 500;
      setTimeout(()=>speak.classList.remove('playing'), est);
    });
    rowDiv.appendChild(speak);
...
function moleculeShape(formula){
  ...
  // MX4 (tetrahedral drawn in 2D as a plus)
  const mx4 = F.match(/^([A-Z][a-z]?)([A-Z][a-z]?)4$/);
  if(mx4) return {
    atoms:[
      {sym:mx4[1],x:0,y:0},
      {sym:mx4[2],x:D,y:0},{sym:mx4[2],x:-D,y:0},
      {sym:mx4[2],x:0,y:D},{sym:mx4[2],x:0,y:-D}
    ],
    bonds:[[0,1,1],[0,2,1],[0,3,1],[0,4,1]]
  };
  // MX5 (trigonal bipyramidal → 3 in a triangle + 2 vertical)
  const mx5 = F.match(/^([A-Z][a-z]?)([A-Z][a-z]?)5$/);
  if(mx5) return {
    atoms:[
      {sym:mx5[1],x:0,y:0},
      {sym:mx5[2],x:D,y:0},
      {sym:mx5[2],x:-D*0.5,y:D*0.87},
      {sym:mx5[2],x:-D*0.5,y:-D*0.87},
      {sym:mx5[2],x:D*0.4,y:D*1.4},
      {sym:mx5[2],x:D*0.4,y:-D*1.4}
    ],
    bonds:[[0,1,1],[0,2,1],[0,3,1],[0,4,1],[0,5,1]]
  };
  // MX6 (octahedral)
  const mx6 = F.match(/^([A-Z][a-z]?)([A-Z][a-z]?)6$/);
  if(mx6) return {
    atoms:[
      {sym:mx6[1],x:0,y:0},
      {sym:mx6[2],x:D,y:0},{sym:mx6[2],x:-D,y:0},
      {sym:mx6[2],x:0,y:D},{sym:mx6[2],x:0,y:-D},
      {sym:mx6[2],x:D*0.6,y:D*0.6},{sym:mx6[2],x:-D*0.6,y:-D*0.6}
    ],
  ...
  if(mno3){
    ...
    return {
      atoms:[
        {sym:mno3[1], x:-D*1.6, y:0},
        {sym:'N', x:0, y:0},
        {sym:'O', x:D, y:-D*0.87},
        {sym:'O', x:D, y:D*0.87},
        {sym:'O', x:-D*0.6, y:0}
      ],
      bonds:[[0,4,1],[1,4,1],[1,2,2],[1,3,1]]
    };
  }
  // Metal di-nitrate M(NO3)2
  const mno32 = F.match(/^([A-Z][a-z]?)\(NO3\)2$/);
  if(mno32){
    // atoms: M plus two nitrate groups
    return {
      atoms:[
        {sym:mno32[1], x:0, y:0},
        // Nitrate 1 (left)
      ...
      bonds:[[0,1,1],[1,2,1],[2,3,2],[2,4,1], [0,5,1],[5,6,1],[6,7,2],[6,8,1]]
    };
  }
  // Metal tri-nitrate M(NO3)3
  const mno33 = F.match(/^([A-Z][a-z]?)\(NO3\)3$/);
  if(mno33){
    const M = mno33[1];
    const groups = [0, 2*Math.PI/3, 4*Math.PI/3];
    const atoms = [{sym:M, x:0, y:0}];
    const bonds = [];
    groups.forEach((theta,g)=>{
      const cx = Math.cos(theta), cy = Math.sin(theta);
      const base = atoms.length;
      atoms.push({sym:'O', x:cx*D*1.4, y:cy*D*1.4});             // O bridge
      atoms.push({sym:'N', x:cx*D*2.3, y:cy*D*2.3});             // N center
      // Two terminal O's perpendicular to the M-O-N axis
      const nx = -cy, ny = cx;
      atoms.push({sym:'O', x:cx*D*3.1 + nx*D*0.7, y:cy*D*3.1 + ny*D*0.7});
      atoms.push({sym:'O', x:cx*D*3.1 - nx*D*0.7, y:cy*D*3.1 - ny*D*0.7});
      bonds.push([0, base, 1]);
      bonds.push([base, base+1, 1]);
      bonds.push([base+1, base+2, 2]);
      bonds.push([base+1, base+3, 1]);
    });
    return {atoms, bonds};
  }
  // Metal hydroxide MOH — already handled above; ensure not clobbered
  // H2SO4 sulfuric acid
  if(F==='H2SO4'){
    return {
      atoms:[
        {sym:'S', x:0, y:0},
        {sym:'O', x:0, y:-D*1.2},                      // =O top
        {sym:'O', x:0, y:D*1.2},                       // =O bottom
        {sym:'O', x:-D*1.2, y:0}, {sym:'H', x:-D*2.2, y:0},  // -O-H left
        {sym:'O', x:D*1.2, y:0},  {sym:'H', x:D*2.2, y:0}    // -O-H right
      ],
  ...
  if(F==='H2SO3'){
    return {
      ...
      bonds:[[0,1,2],[0,2,1],[2,3,1],[0,4,1],[4,5,1]]
    };
  }
  // HAuCl4 (tetrachloroauric acid) — square planar
  if(F==='HAuCl4'){
    return {
      atoms:[
        {sym:'Au', x:0, y:0},
        {sym:'Cl', x:D*1.4, y:0}, {sym:'Cl', x:-D*1.4, y:0},
        {sym:'Cl', x:0, y:D*1.4}, {sym:'Cl', x:0, y:-D*1.4},
        {sym:'H',  x:D*2.4, y:0}
      ],
...
function chainMolecule(formula){
  // Real chemical-formula tokenizer with parenthesis + multiplier support.
  // Returns { atoms:[{sym,x,y}], bonds:[[i,j,order]] }
...
/* Expand a formula like "Cu(NO3)2" into a list of atom symbols. */
function expandFormula(formula){
  const out = [];
  // Parenthesis pass: recursively expand (…)_n groups
  // We build a mini-parser: read Symbol|Group[Number]* and push atoms.
  let i = 0;
  function readAtoms(){
    const local = [];
    while(i < formula.length){
      ...
      if(ch === '(' ){
        ...
        for(let k=0;k<mult;k++) local.push(...sub);
      } else if(ch === ')'){
        // Return to caller
        return local;
      } else if(/[A-Z]/.test(ch)){
        ...
        const mult = readNumber();
        for(let k=0;k<mult;k++) local.push({sym});
      } else {
        // skip anything else (space, dot, etc.)
        i++;
      }
    }
    return local;
...
async function speak(text, langHint){
  ...
  // items in the queue and truncates the last utterance. The caller
  // (speakSequence / TTS button) handles cancellation.
  ...
  return new Promise(resolve=>{
    ...
    // Watchdog: if the browser never fires onend (a real Chrome/Edge
    // bug on long queues), force-resolve after a generous timeout.
    const wordCount = Math.max(1, (text || '').trim().length);
    const guardMs = Math.min(8000, 800 + wordCount * 300);
...
async function speakSequence(zList, lang){
  ...
  for(const z of zList){
    ...
    // tiny pause between names
    await new Promise(r=>setTimeout(r, 30));
  }
}
document.getElementById('ttsEn').addEventListener('click',(e)=>{
  if(!currentZ) return;
  const button = e.currentTarget;             // currentTarget is null once dispatch ends
