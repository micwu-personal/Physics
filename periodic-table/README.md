...
4. **Bilingual with traceable terminology.** Chinese physics/chemistry vocabulary follows 全国科学技术名词审定委员会 terminology, with the official CNCTST portal linked in the interface.
...
Z, mass, group, period, family, electron configuration, shell occupancy (K L M N O P Q), phase at STP, melting/boiling points, density, Pauling electronegativity, and separate stable / naturally radioactive / trace-natural / synthetic radioactivity classes. Superheavy bulk phases are shown as unmeasured rather than silently inferred. Helium's melting point carries its pressure caveat.
...
- **Bond-direction models** — sp (180° linear), sp² (120° trigonal planar), sp³ (109.5° tetrahedral), trigonal-bipyramidal and octahedral geometry, plus textbook transition-metal geometry labels.

All shapes are rendered as 3-D projections with perspective, phase-colored (cyan `+`, pink `−` for the wavefunction sign), painter's-algorithm z-sorted, drag-to-rotate, and auto-spin when active. PCl₅ and SF₆ explicitly identify `sp³d`/`sp³d²` as legacy geometry labels and link modern hypervalent-bonding literature.
...
Only reviewed element-specific examples are shown; unsupported category-wide formula generation was removed. Every included equation is atom-balance-validated. Click ▶ Play to see a 4-phase animation:
...
Animated nucleus with protons (gold) and neutrons (grey) packed in a Fibonacci-spiral pattern with gentle wobble. Reviewed isotope records show abundance, stability, and notes; missing records remain blank rather than being replaced by a rounded-mass isotope at an invented abundance.
...
Every element carries a deliberately simplified dominant-contribution classification (`F_ORIGIN`): **Big Bang · cosmic-ray spallation · AGB s-process · massive-star fusion · type-Ia supernovae · core-collapse supernovae · mixed neutron-capture production · r-process · predominantly human production**. The interface states that many elements have multiple, metallicity-dependent origins and that r-process site shares remain model-dependent.
...
5. `r-process events forge heavy nuclei` — with no claim that every r-process nucleus has one exclusive astrophysical site
...
Each element flashes as its simplified dominant production era is reached. Mixed s/r-process elements are grouped with the AGB era, while the dedicated r-process era remains explicitly model-dependent.
...
- **eka-manganese** → technetium (Tc, 1937) — identified 30 years after Mendeleev's death in 1907
...
A canvas draws the octahedral splitting diagram (three t₂g levels below, two e_g above) using the approximate Jørgensen `Δ = f · g` model. The interface labels the single-band wavelength and sRGB-complement display as an approximation because real spectra can contain multiple transitions, vibronic structure, and non-sRGB color mixing.
...
├── science.js          Pure shell, formula, reaction, lifecycle helpers
├── source-registry.js  Structured authoritative references + safe link renderer
...
├── package.json        Build, validation, coverage, and profiling scripts
├── validate.js         Dependency-free scientific/data invariant validation
├── profile.js          CPU/memory and animation-lifecycle audit
...
- **Rich data for an element** — add an entry keyed by Z to `EXTENDED` in `data.js`. The fallback supplies only explicitly safe category text and leaves unsupported isotope/reaction values absent.
...
- **Reviewed reaction** — add a balanced, sourced entry to `EXTENDED` or `ELEMENT_REACTIONS`; category-wide reaction generation is intentionally unsupported.
...
## Scientific references and limitations

Every key interface section renders its own safe external reference links from `source-registry.js`; references are not hidden behind a general footer. The registry covers core properties, shells, orbitals/hypervalent bonding, oxidation states, colors, reactions, molecular geometry, isotopes, discovery, cosmic origins, nuclides, overlays, and ligand-field modeling. Primary/curated sources include IUPAC, CIAAW, NIST, NNDC NuDat, IAEA LiveChart, CNCTST, RSC, and peer-reviewed DOI links. The validation suite asserts that every content group has source coverage.

The application is educational rather than an evaluated atomic/nuclear database. Superheavy configurations and phases, empirical radii, ligand-field colors, and nucleosynthetic contributions can be theoretical or model-dependent; those approximations are labeled in the corresponding interface.
