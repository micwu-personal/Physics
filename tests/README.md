...
Particle Zoo ships a persistent motion override: `?motion=play|pause|system` wins over the stored `pz-motion` preference, an accepted override is written back (and cleared again for `system`), and `system` follows the platform reduced-motion setting. `coverage.spec.js` drives each of those states through the real page — one journey per query value, one for a stored `pause` restored at boot, one for `system` while the platform preference flips, and the storage-denied journey now refuses both `pz-lang` and `pz-motion` — and asserts the observable result: the toggle's `data-state`, its localized label and system-override title, `html[data-motion]`, what `localStorage` holds afterwards, and whether the lab and interaction frame counters advance.
